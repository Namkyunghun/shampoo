...
def train(args: argparse.Namespace):
    ...
    print(f"Running DDP training with AdamW. Global Rank: {global_rank}, Local Rank: {local_rank}, World Size: {world_size}")

    writer = SummaryWriter(log_dir=args.log_dir) if global_rank == 0 else None

    # --- 데이터 증강 및 Mixup 설정 (timm 사용) ---
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Vision Transformer on ImageNet with DDP, AdamW, and Algoperf Augmentations')
    # 기본 인자
    ...
    # 데이터 증강 관련 인자
    parser.add_argument('--mixup', type=float, default=0.2, help='Mixup alpha. Set 0 to disable.')
    parser.add_argument('--label-smoothing', type=float, default=0.1, help='Label smoothing')

    # [변경] AdamW 옵티마이저 하이퍼파라미터 인자
    parser.add_argument('--weight-decay', type=float, default=0.001, help='Weight decay for AdamW')
    parser.add_argument('--beta1', type=float, default=0.95, help='AdamW beta1 parameter')
